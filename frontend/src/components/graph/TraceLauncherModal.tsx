      ...
      <div className="w-full max-w-lg bg-police-900 border border-police-700 rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="p-5 border-b border-police-800 bg-police-800/60 flex items-center justify-between">
          ...
          <button
            onClick={onClose}
            disabled={loading}
            className="p-1 rounded-lg text-surface-400 hover:text-white hover:bg-police-800 transition disabled:opacity-50"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        {/* Error Alert */}
        {error && (
          <div className="m-4 p-3 rounded-lg bg-red-950/60 border border-red-800 text-red-200 text-base flex items-center gap-2">
        ...
        {/* Form */}
        <form onSubmit={handleSubmit} className="p-5 space-y-4 text-base">
          {/* Target Address */}
          <div className="space-y-1.5">
            <label className="block text-surface-300 font-semibold uppercase text-sm tracking-wider">
            ...
            <input
              ...
              className="w-full px-3 py-2 bg-police-800 border border-police-700 rounded-lg text-slate-100 font-mono placeholder-surface-400 focus:outline-none focus:border-brand-blue"
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <label className="block text-surface-400 font-semibold uppercase text-sm tracking-wider">
                Blockchain
              </label>
              <input
                type="text"
                disabled
                value="TRON Mainnet"
                className="w-full px-3 py-2 bg-police-950/80 border border-police-800 rounded-lg text-surface-400 font-medium cursor-not-allowed"
              />
            </div>
            <div className="space-y-1.5">
              <label className="block text-surface-400 font-semibold uppercase text-sm tracking-wider">
          ...
          <div className="space-y-1.5">
            <div className="flex justify-between items-center">
              <label className="text-surface-300 font-semibold uppercase text-sm tracking-wider flex items-center gap-1.5">
                <Layers className="h-4 w-4 text-brand-blue" />
                Maximum Traversal Depth (Hops)
              </label>
              <span className="font-mono font-bold text-brand-blue">{maxHops} Hops</span>
            </div>
            <input
              type="range"
              min="1"
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <span className="text-surface-400 font-semibold">USD</span>
            </div>
          ...
          <div className="pt-3 border-t border-police-800 flex items-center justify-end gap-2">
            <button
              ...
              onClick={onClose}
              disabled={loading}
              className="px-4 py-2 rounded-lg bg-police-800 hover:bg-police-700 text-surface-300 transition"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={loading}
              className="px-5 py-2 rounded-lg bg-brand-blue hover:bg-brand-hover text-white font-bold transition flex items-center gap-2 shadow-lg shadow-blue-500/20 disabled:opacity-50"
            >
              {loading ? (
                <>

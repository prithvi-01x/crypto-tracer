...
export const GraphDetailDrawer: React.FC<GraphDetailDrawerProps> = ({
  ...
  return (
    <div className="w-80 lg:w-96 bg-police-850/95 border-l border-police-700/80 shadow-2xl flex flex-col h-full backdrop-blur transition-all duration-200">
      ...
      <div className="p-4 border-b border-police-700/80 flex items-center justify-between bg-police-800/60">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <span className="text-[10px] text-slate-400">Forensic Graph Item</span>
          </div>
        </div>
        <button
          onClick={onClose}
          className="p-1 rounded-md text-slate-400 hover:text-white hover:bg-police-700 transition"
        >
          <X className="h-4 w-4" />
        </button>
      </div>
        ...
        {selectedNode && (
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="px-2 py-0.5 rounded text-[10px] font-mono bg-police-900 text-slate-300 border border-police-700">
            ...
            <div className="p-3 rounded-lg bg-police-900/90 border border-police-700/80 space-y-1.5">
              <div className="text-[10px] text-slate-400 uppercase font-semibold">Wallet Address</div>
              <div className="font-mono text-xs text-emerald-300 break-all select-all leading-tight">
            ...
            <div className="p-3 rounded-lg bg-police-900/70 border border-police-700/70 space-y-2.5">
              <div className="text-[10px] text-slate-400 uppercase font-semibold">Observed Fund Flows</div>
              <div className="grid grid-cols-2 gap-2 text-xs">
                ...
                <div>
                  <span className="text-[10px] text-slate-500 block">Total Sent Out</span>
                  <span className="font-mono font-bold text-red-300">
                    {formatUsdt(selectedNode.total_sent)}
                  </span>
                </div>
              </div>
              <div className="pt-1 border-t border-police-800 flex justify-between items-center text-[11px]">
                <span className="text-slate-400">Observed Transactions:</span>
            ...
            {onHighlightPathToRoot && selectedNode.node_type !== 'suspect' && (
              <button
                ...
                className="w-full py-2 px-3 rounded-lg bg-blue-600 hover:bg-blue-500 text-white font-semibold text-xs transition flex items-center justify-center gap-1.5 shadow-md shadow-blue-500/20 cursor-pointer"
              >
        ...
        {selectedEdge && (
          <div className="space-y-4">
            ...
            <div className="p-3.5 rounded-lg bg-police-900/90 border border-police-700/80 space-y-1">
              <div className="text-[10px] text-slate-400 uppercase font-semibold">Transfer Value</div>
            ...
            <div className="p-3 rounded-lg bg-police-900/70 border border-police-700/70 space-y-2">
              <div className="text-[10px] text-slate-400 uppercase font-semibold">Directional Route</div>
              <div className="space-y-1.5 font-mono text-[11px]">
                <div className="flex items-center justify-between">
                  <span className="text-slate-500 text-[10px]">From:</span>
                  <span className="text-slate-200">{selectedEdge.from_address.slice(0, 10)}...{selectedEdge.from_address.slice(-6)}</span>
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-slate-200">{selectedEdge.to_address.slice(0, 10)}...{selectedEdge.to_address.slice(-6)}</span>
                </div>
              </div>
            </div>
            ...
            <div className="p-3 rounded-lg bg-police-900/70 border border-police-700/70 space-y-1.5">
              <div className="text-[10px] text-slate-400 uppercase font-semibold">Transaction Hash</div>
              <div className="font-mono text-xs text-slate-300 break-all select-all leading-tight">
            ...
            <div className="grid grid-cols-2 gap-2 text-[11px]">
              <div className="p-2.5 rounded-lg bg-police-900/60 border border-police-700/60">
                <span className="text-[10px] text-slate-500 block">Block Height</span>
              ...
              <div className="p-2.5 rounded-lg bg-police-900/60 border border-police-700/60">
                <span className="text-[10px] text-slate-500 block">Provenance Source</span>
                <span className="font-mono font-medium text-slate-300">{selectedEdge.source}</span>
              </div>
            </div>

            <div className="p-2.5 rounded-lg bg-police-900/60 border border-police-700/60 text-[11px] flex items-center justify-between">
              <span className="text-slate-400">Timestamp:</span>
              <span className="font-mono text-slate-300">{new Date(selectedEdge.timestamp).toLocaleString()}</span>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="p-3 border-t border-police-700/80 bg-police-900 flex items-center justify-between text-[10px] text-slate-400">

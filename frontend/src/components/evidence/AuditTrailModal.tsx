...
export const AuditTrailModal: React.FC<AuditTrailModalProps> = ({
  ...
  const getEventBadgeClass = (eventType: string) => {
    switch (eventType) {
      ...
      default:
        return 'bg-slate-700/50 text-surface-300 border-slate-600/50';
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-in fade-in duration-150">
      <div className="w-full max-w-3xl max-h-[85vh] flex flex-col rounded-2xl bg-police-900 border border-police-700 shadow-2xl overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-lg font-bold text-white tracking-wide">Investigator Chain of Custody & Audit Trail</h3>
        ...
        {/* Filter Bar */}
        <div className="px-6 py-2.5 bg-police-950/60 border-b border-police-800 flex items-center justify-between text-base">
          <div className="flex items-center gap-2 text-surface-400">
            ...
            <select
              ...
              className="px-2 py-1 rounded bg-police-900 border border-police-700 text-base font-medium text-surface-200 focus:outline-none focus:border-brand-blue"
            >
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-3">
          {loading ? (
            <div className="flex flex-col items-center justify-center p-12 space-y-3">
              ...
              <span className="text-base text-surface-400">Retrieving append-only audit events...</span>
            </div>
          ) : error ? (
            <div className="p-4 rounded-xl bg-red-500/10 border border-red-500/20 text-base text-red-300 flex items-center gap-2">
              <AlertCircle className="h-4 w-4 shrink-0 text-red-400" />
              <span>{error}</span>
            </div>
          ) : filteredEvents.length === 0 ? (
            <div className="text-center py-12 text-surface-500 text-base">
            ...
            filteredEvents.map((ev) => (
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2">
                    <span className={`px-2 py-0.5 rounded-full text-sm font-semibold tracking-wider border ${getEventBadgeClass(ev.event_type)}`}>
                      {ev.event_type.replace(/_/g, ' ')}
                    </span>
                    <span className="text-base font-semibold text-surface-200">
                      {ev.action_summary}
                    </span>
                  </div>
                  <div className="flex items-center gap-1.5 text-base text-surface-400 font-mono shrink-0">
                    <Clock className="h-4 w-4" />
                    <span>{new Date(ev.created_at).toLocaleString()}</span>
                  </div>
                </div>

                <div className="flex flex-wrap items-center justify-between gap-2 pt-2 border-t border-police-800 text-base">
                  <div className="flex items-center gap-3 text-surface-400">
                    <span className="flex items-center gap-1">
                      <User className="h-4 w-4 text-surface-500" />
                      Actor: <strong className="text-surface-300 font-mono">{ev.actor_id}</strong>
                    </span>
                    {ev.trace_id && (
                      <span className="text-surface-500">
                        Trace: <span className="font-mono text-surface-400">{ev.trace_id.slice(0, 8)}...</span>
                      </span>
                    )}
                  </div>

                  {/* Tamper-evident Hash */}
                  <div className="flex items-center gap-1.5 font-mono text-sm bg-police-950 px-2 py-0.5 rounded border border-police-800 text-surface-400">
